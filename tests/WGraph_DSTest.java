...
class WGraph_DSTest {
    ...
    //add 5000 nodes connect even to odd and remove 50 edge
    @Test
    void AddEdge_Remove(){
        weighted_graph g = new WGraph_DS();
        int V=5000;
        ...
        for (int i=0; i<V; i++) {
            ...
            if (i%2==0){
                ...
                g.connect(i,i+1, Math.random());
                g.connect(i,i+1, Math.random());

            }
        }
        int expected = g.edgeSize();
        assertEquals(expected, 2500);
        int j=0;
        if (g.hasEdge(0,1)) {
            System.out.println("has edge before");
            g.removeEdge(0, 1);
            if (g.hasEdge(0,1))
                System.out.println("has edge after");
        }
        while(j<50) {
            if (g.hasEdge(j, j + 1)) {
